...
char            fill_elf_header(t_elf_file *file) {
    ...
    return (file->file_infos.st_size >= sizeof(Elf32_Ehdr));
}
...
char                fill_elf_sections(t_elf_file *file) {
    ...
    if (!file->is_32bits) {
        ...
        return 1;
//        return (file->mapped_mem + sizeof(Elf64_Shdr) * file->elf_header->e_shnum < file->end);
    }
    ...
    if (file->elf_sections > file->end - file->elf_header->e_phnum * sizeof(Elf32_Shdr))
        return 0;
    i = 0;
...
char fill_symtabs(Elf64_Sym **sym_tabs64, Elf32_Sym *sym_tabs32, int nbr_symtabs) {
    ...
    i = 0;
    while (i < nbr_symtabs) {
        (*sym_tabs64)[i].st_name = sym_tabs32[i].st_name;
        (*sym_tabs64)[i].st_value = sym_tabs32[i].st_value;
        (*sym_tabs64)[i].st_size = sym_tabs32[i].st_size;
        (*sym_tabs64)[i].st_info = sym_tabs32[i].st_info;
        (*sym_tabs64)[i].st_other = sym_tabs32[i].st_other;
        (*sym_tabs64)[i].st_shndx = sym_tabs32[i].st_shndx;
        i++;
    }
    return 1;
}